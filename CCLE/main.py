...
target_drug_cids = np.array([5330286], dtype=np.int32)
...
null_mask = np.zeros_like(cell_drug)
...
for fold in range(n_kfolds):
    ...
    for train_data, test_data, train_mask, test_mask in sampler(dtype=dtype, device=device):
        ...
        opt = ModelOptimizer(model=model, epochs=1500, lr=5e-4, test_data=test_data, test_mask=test_mask)
